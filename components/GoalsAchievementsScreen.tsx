...
const GoalsAchievementsScreen: React.FC<GoalsAchievementsScreenProps> = ({
  ...
  return (
    <div className="animate-fadeIn min-h-screen bg-transparent pb-20">
      ...
      <div className="p-4">
        <div className="flex bg-zinc-900/50 backdrop-blur-sm p-1 rounded-xl mb-6 border border-zinc-800">
          <button 
          ...
          <div className="space-y-4 animate-fadeIn">
            {showAddGoal ? (
              ...
              <button 
                ...
                className="w-full py-4 border border-dashed border-zinc-700 bg-zinc-900/20 rounded-2xl text-zinc-500 font-bold uppercase text-xs hover:border-red-600 hover:text-red-500 transition-all flex items-center justify-center gap-2"
              >
            ...
            {student.goals.map(goal => (
              <div key={goal.id} className={`p-5 rounded-2xl border transition-all backdrop-blur-sm ${goal.completed ? 'bg-green-900/30 border-green-900/30' : 'bg-zinc-900/60 border-zinc-800'}`}>
        ...
        {activeTab === 'ACHIEVEMENTS' && (
          <div className="grid grid-cols-2 gap-4 animate-fadeIn">
            {student.achievements.map(ach => (
              <div key={ach.id} className={`p-4 rounded-2xl border flex flex-col items-center text-center gap-3 relative overflow-hidden backdrop-blur-sm ${ach.unlocked ? 'bg-gradient-to-br from-zinc-800/80 to-zinc-900/80 border-yellow-600/30' : 'bg-zinc-900/30 border-zinc-800 opacity-50 grayscale'}`}>
